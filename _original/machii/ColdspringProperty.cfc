...
<cfcomponent
	...
	<cffunction name="configure" access="public" returntype="void" output="false"
		...
		<cfscript>
			 var logArgs={
				...
				localBeanFactoryKey:localBeanFactoryKey,
				defaultProperties:defaultProperties.filter(function(key,value){
					return (!(isObject(value)));
				}),
				defaultAttributes:defaultAttributes.filter(function(key,value){
					return (!(isObject(value)));
				})
			};
		</cfscript>
		...
		<cftry>
			...
			<cfcatch type="any">
				...
				<cfthrow type="MachII.properties.ColdSpringProperty.LoadBeansFromXmlFileException"
					...
					extendedInfo="#SerializeJSON(logArgs)#"
				/>
			</cfcatch>
		</cftry>
	...
	<cffunction name="autowireByDynamicMethodGeneration" access="private" returntype="void" output="false"
		...
		<cfscript>
			var logArgs=arguments.filter(function(key,value){
				return (!(isObject(value)));
			});
		</cfscript>
		<cfset var beanFactory = getProperty(getProperty("beanFactoryName")) />
